...
const Header = () => {
  ...
  return (
    <header className="p-6 w-96 h-[450px] dark:bg-slate-800 grid place-items-center grid-rows-2 grid-cols-[100px,1fr]">
      <img
        src={Perfil}
        alt="Avatar del usuario"
        className="h-48 w-48 rounded-full mt-24 border-2 border-slate-400 bg-slate-400 mb-2"
      />
      <article className="h-36 w-full mt-20 flex flex-col gap-3 ml-12">
        <h1 className="text-3xl font-semibold text-slate-300 hover:brightness-200">
          Enzo Peralta
        </h1>
        <h2 className="font-semibold text-slate-300 hover:brightness-200">
          {texto}|
        </h2>
      </article>
      <ul className="row-start-1 row-end-3 col-start-1 h-full w-full flex flex-col gap-5 justify-center">
        {dataNetwork.map((e, i) => (
          <li key={i}>
            <Icon
              path={e.path}
              className="text-slate-400 hover:brightness-200 duration-75 hover:translate-x-1 transition-all"
            />
          </li>
        ))}
      </ul>
